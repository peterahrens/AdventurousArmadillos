...
void noflagOCC_solver(double wxt, std::complex<double> *wtilde_array, int my_igp, int n1, std::complex<double> *aqsmtemp, std::complex<double> *aqsntemp, std::complex<double> *I_eps_array, std::complex<double> &ssxt, std::complex<double> &scht, int ncouls, int igp, std::complex<double> *scha)
{
    ...
    double limittwo = pow(0.5,2);
    std::complex<double> mygpvar1 = std::conj(aqsmtemp[n1*ncouls+igp]);
    double scht_loc_real = 0.0;
    double scht_loc_imag = 0.0;
    
    //#pragma omp parallel for schedule(static) reduction(+:scht_loc_real,scht_loc_imag)
    for(int ig = 0; ig<ncouls; ++ig)
    {
        ...
        std::complex<double> scht_loc = mygpvar1 * aqsntemp[n1*ncouls+ig] * delw * I_eps_array[my_igp*ncouls+ig] ;
        scht_loc_real += std::real(scht_loc);
        scht_loc_imag += std::imag(scht_loc);
    }

    scht = std::complex<double>(scht_loc_real, scht_loc_imag);
}
...
//The main work starts here
    for(int n1 = 0; n1<number_bands; ++n1) 
    {
        ...
        double acht_n1_loc_loc_real = 0;
        double achtemp_loc_0_real = 0;
        double achtemp_loc_1_real = 0;
        double achtemp_loc_2_real = 0;
        double achtemp_loc_3_real = 0;
        double acht_n1_loc_loc_imag = 0;
        double achtemp_loc_0_imag = 0;
        ...
        for(int my_igp=0; my_igp<ngpown; ++my_igp)
        {
            ...
            std::complex<double> acht_n1_loc_loc = sch_array[2] * vcoul[igp];
            std::complex<double> achtemp_loc_0 = sch_array[0] * vcoul[igp];
        ...
        achtemp[2] += std::complex<double>(achtemp_loc_2_real, achtemp_loc_2_imag);
        achtemp[3] += std::complex<double>(achtemp_loc_3_real, achtemp_loc_3_imag);

        acht_n1_loc[n1] += std::complex<double>(acht_n1_loc_loc_real, acht_n1_loc_loc_imag);
    }
    //Time Taken
